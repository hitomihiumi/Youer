...
public class ItemAPI {
    ...
    public static String getNbtAsString(CompoundTag compoundTag) {
        return compoundTag == null ? "null" : compoundTag.toString(); // Youer - Tag#getAsString is gone; asString() returns an Optional in 1.21.5+
    }

    public static String getNbtAsString(ItemStack itemStack) {
        var item = CraftItemStack.asNMSCopy(itemStack);
        try {
            net.minecraft.nbt.CompoundTag nbt = saveItem(item, net.minecraft.core.RegistryAccess.EMPTY); // Youer - ItemStack#save is gone in 1.21.5
    ...
    public static TextComponent show(ItemStack itemStack) {
        ...
        compound.merge(saveItem(nmsItemStack, MinecraftServer.getServer().registryAccess())); // Youer - ItemStack#save is gone in 1.21.5
    ...
    public static Material get(ResourceLocation key) {
        return BuiltInRegistries.ITEM.getValue(key).getDefaultInstance().asBukkitCopy().getType(); // Youer - Registry#get returns an Optional since 1.21.2
    }
    ...
    // Youer - ItemStack#save was removed in 1.21.5; encode through the codec, the way CraftMagicNumbers does
    public static net.minecraft.nbt.CompoundTag saveItem(net.minecraft.world.item.ItemStack item, net.minecraft.core.HolderLookup.Provider registries) {
        return (net.minecraft.nbt.CompoundTag) net.minecraft.world.item.ItemStack.CODEC
            .encodeStart(registries.createSerializationContext(net.minecraft.nbt.NbtOps.INSTANCE), item)
            .getOrThrow();
    }
}
