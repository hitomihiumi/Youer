...
public class NeoForgeInjectBukkit {
    ...
    // Youer - addEnumArt is gone: org.bukkit.Art is no longer an enum but a registry-backed interface
    // (Paper's OldEnum) in 1.21.8, so a modded painting variant is already reachable through
    // RegistryAccess and there is nothing to inject. MODD_ART stays empty.
