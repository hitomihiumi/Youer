    ...
    public static EntityDamageEvent handleEntityDamage(LivingEntity livingEntity, final DamageSource damagesource, float f) {
        ...
        Function<Double, Double> blocking = new Function<Double, Double>() {
            ...
            public Double apply(Double f) {
                // Youer - 1.21.5 replaced LivingEntity#isDamageSourceBlocked with the blocks-attacks
                // component; this mirrors what CraftBukkit's own blocking modifier now does
                if (!livingEntity.canBlockAttack(damagesource, f.floatValue())) {
                    return 0.0;
                }
                return (double) -livingEntity.resolveBlockedDamage(damagesource, f.floatValue());
            }
        };
        ...
        Function<Double, Double> armor = new Function<Double, Double>() {
            @Override
            public Double apply(Double f) {
                // Youer - no calDamageEvent guard needed: getDamageAfterArmorAbsorb no longer damages armour
                // (CraftBukkit moved that into actuallyHurt), so simulating it here has no side effect
