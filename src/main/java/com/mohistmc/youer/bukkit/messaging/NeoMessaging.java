...
public class NeoMessaging {
    ...
    public static PayloadRegistration<?> createRegistration(PluginChannel<NeoForgePayloadHandler> channel) {
        ...
        // Youer - NeoForge 21.8 took the handler out of PayloadRegistration and keeps it in
        // NetworkRegistry's SERVERBOUND_HANDLERS/CLIENTBOUND_HANDLERS instead, so register it there.
        var registration = new PayloadRegistration<>(type, codec, PluginChannel.PROTOCOLS, Optional.ofNullable(flow), CHANNEL_VERSION, true);
        for (var protocol : PluginChannel.PROTOCOLS) {
            if (registration.matchesFlow(PacketFlow.SERVERBOUND)) {
                NetworkRegistry.SERVERBOUND_HANDLERS.get(protocol).put(type.id(), handler);
            }
            if (registration.matchesFlow(PacketFlow.CLIENTBOUND)) {
                NetworkRegistry.CLIENTBOUND_HANDLERS.get(protocol).put(type.id(), handler);
            }
        }
        return registration;
    }
